...
function principal(){
    ...
    let mesesADR = Number(document.getElementById('mesesADR').value)
    let taxaADR = Number(document.getElementById('taxaADR').value)
    let depositoRegular = Number(document.getElementById('depositoRegular').value)
    let valorFInalADR = calcularADR(mesesADR,taxaADR,depositoRegular)
    console.log(`O valor Final é: ${valorFInalADR.toFixed(2)}`)

    resultado.innerHTML = ``
    resultado.innerHTML += `O valor Final é: ${valorFInalADR.toFixed(2)} <br>`
